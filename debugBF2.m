% This scrip load the minimum to compare both method of spectrogram
...
for u=1:nbV
    ...
    [~,freqV,timeV,mat1] = spectrogram(recon(:,u),win.val,win.novlp,win.nfft,spec.Fs); 
...
for u=1:2
    ...
    %Plot
    if u==1
    pcolor(timeVC + timeWin(1), freqVC,(squeeze(reconFFTC(aziPrint,:,:)))'); shading flat;
    else
        pcolor(timeV + timeWin(1), freqV,(squeeze(reconFFT(aziPrint,:,:)))'); shading flat;
